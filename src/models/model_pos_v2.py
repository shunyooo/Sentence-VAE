...
class POSVAE(nn.Module):
    ...
    def __init__(self, vocab_size, pos_vocab_size, embedding_size, pos_embedding_size, rnn_type, hidden_size, 
        ...
        self.latent2text_decoder_hidden = nn.Linear(latent_size+hidden_size, hidden_size * self.hidden_factor)
    ...
    def forward_pos2text(self, text_sequence, text_length, pos_sequence, pos_length, z):
        ...
        text_decoder_hidden = self.latent2text_decoder_hidden(torch.cat([z, pos_hidden], dim=1))
        # Text Decode
        text_logp = self.text_decode_batch(text_decoder_hidden, text_sequence, text_length)
        return {'init_state': init_state, 'pos_hidden': text_decoder_hidden, 'text_logp': text_logp}
    ...
    def text_inference(self, pos_sequence, pos_length, z):
        ...
        text_decoder_hidden = self.latent2text_decoder_hidden(torch.cat([z, pos_hidden], dim=1))
        # Decode
        return self._inference(batch_size, text_decoder_hidden, self.embedding, self.text_decoder_rnn, self.outputs2vocab, self.tgt_max_sequence_length)
